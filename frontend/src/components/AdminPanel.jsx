...
export default function AdminPanel({ activePanel }) {
  ...
  useEffect(() => {
    ...
    if (activePanel.includes('view') || activePanel.includes('manage')) {
      const role = activePanel.replace('view', '').replace('manage', '').replace('-', '');
      fetchUsers(role === 'siswa' ? 'murid' : role);
    }
  ...
  const fetchStats = async () => {
    try {
      ...
      setStats({
        siswa: siswa.data.length,
        petugas: petugas.data.length,
        walas: walas.data.length,
        admin: admin.data.length
      });
    } catch (err) {
      console.error(err);
    }
  ...
  const handleEdit = async (e) => {
    ...
    try {
      ...
      const role = activePanel.replace('manage-', '').replace('-panel', '');
      fetchUsers(role === 'siswa' ? 'murid' : role);
    } catch (err) {
      ...
      <div className="panel active-panel">
        ...
        <div className="stats-grid">
          <div className="stat-card">
            <div className="stat-info"><h3>Siswa</h3><div className="number">{stats.siswa}</div></div>
            <i className="fas fa-user-graduate stat-icon"></i>
          </div>
          <div className="stat-card">
            <div className="stat-info"><h3>Petugas</h3><div className="number">{stats.petugas}</div></div>
            <i className="fas fa-chalkboard stat-icon"></i>
          </div>
          <div className="stat-card">
            <div className="stat-info"><h3>Wali Kelas</h3><div className="number">{stats.walas}</div></div>
            <i className="fas fa-chalkboard-user stat-icon"></i>
          </div>
          <div className="stat-card">
            <div className="stat-info"><h3>Admin</h3><div className="number">{stats.admin}</div></div>
          ...
          <>
            ...
            <div className="form-group">
              ...
              <select name="wali_kelas_id">
                ...
                {walasList.map(w => (
                  <option key={w._id} value={w._id}>{w.nama_lengkap}</option>
                ))}
              </select>
            </div>
          </>
      ...
      <div className="modal active">
        <div className="modal-content">
          ...
          <form onSubmit={handleEdit}>
            ...
            <div className="form-group">
              <label>Nama</label>
              <input type="text" name="nama_edit" defaultValue={editData.nama_lengkap} required />
            </div>
            <div className="form-group">
              <label>NIK</label>
              <input type="text" name="nik_edit" defaultValue={editData.nik} required />
            </div>
            <div className="form-group">
              <label>NIS</label>
              <input type="text" name="nis_edit" defaultValue={editData.nis || ''} />
            </div>
            <div className="form-group">
              <label>Tgl Lahir</label>
              <input type="date" name="tanggal_lahir_edit" defaultValue={editData.tanggal_lahir ? editData.tanggal_lahir.split('T')[0] : ''} />
            </div>
            <div className="form-group">
              <label>Role</label>
